document.addEventListener("DOMContentLoaded", function() {
    ...
    const carouselsData = [
        { id: 'carousel-inner', indicatorsId: 'carousel-indicators', plans: [] },
    ];
    ...
    function renderCarousels() {
        carouselsData.forEach((carouselData, index) => {
            carouselData.plans = plans.filter(plan => plan.id === "1").slice(index * 10, (index + 1) * 10);
    ...
    function renderCarousel(carouselData) {
        ...
        carouselInner.innerHTML = carouselData.plans.map(plan => {
            ...
            return `<div class="carousel-item"><a href="${BASE_URL}/planos#${plan.linkrelativo}"><img src="${imgSrc}" alt="${plan.Título}"></a></div>`;
        }).join('');
